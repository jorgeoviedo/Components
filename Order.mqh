...
void Order::orderCheckModSLTP() {
     ...
     if ((accountProfit > 0) && (accountProfit > lastAccProfit) && 
         (PositionsTotal() > 0)) {
          orderModifySLTP();
          if (GetLastError() == 0) {
...
void Order::orderGetInfoOnTick() {
     ...
     SymbolInfoDouble(_Symbol, SYMBOL_BID), lastAccProfit,
     SymbolInfoInteger(_Symbol, SYMBOL_SPREAD), 
     AccountInfoDouble(ACCOUNT_EQUITY), AccountInfoDouble(ACCOUNT_PROFIT)));
}
//+---------------------------------------------------------------------------+
//| orderGetEventTimer()                                                      |
//+---------------------------------------------------------------------------+
...
void Order::orderPaintText() {
     ...
     ObjectSetInteger(0, objName, OBJPROP_COLOR, clrYellow);
     ObjectSetString (0, objName, OBJPROP_TEXT, request.comment + 
